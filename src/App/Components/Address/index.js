...
export default function Address(props) {
    ...
    const settingDeliveryAddress = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                const addressForString = availableAddresses.filter(
                    (e) => e.addressId === defaultAddress
                );
        ...
        <Box display="flex" flexDirection="column" gap="1rem">
         ...
         setAddressState={setAddressState} />) : ''} */}
            ...
            {!profilePage && availableAddresses.length > 0 && (
                <FormControl>
                    <RadioGroup name="address" value={defaultAddress} onChange={handleChange}>
                        {availableAddresses.map((data) => (
                            <FormControlLabel
                                    ...
                                    <>
                                        ...
                                        {defaultAddress === data.addressId && (
                                            <button
                                                ...
                                                disabled={loading}
                                                onClick={settingDeliveryAddress}
                                            >
                                                DELIVER HERE {loading && <CircularProgress />}
                                            </button>
                                        )}
                                    </>
                                }
