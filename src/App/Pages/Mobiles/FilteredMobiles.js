...
export default function FilteredMobiles() {
  ...
  const getMobileProperties = async () => {
    try {
      ...
      const response = await fetch(
        `${getCategoryPropertiesUrl}353b54c1-3ee2-4aee-8923-686927e4db9e`,
        options,
      );
      if (response.ok) {
  ...
  const getMobilesData = async () => {
    ...
    try {
      ...
      const url = isThisFilterPage
        ? `${mobilesFilterUrl}?${query}&page=${page}`
        : `${searchUrl}${searchValue}&page=${page}`;
      ...
      if (response.ok) {
        ...
        setTotalPages(
          isThisFilterPage
            ? Math.ceil(totalFilterResults / 20)
            : Math.ceil(totalSearchResults / 20),
        );
      }
  ...
  const getBrands = async () => {
    try {
      ...
      const response = await fetch(
        `${getBrandsUrl}353b54c1-3ee2-4aee-8923-686927e4db9e`,
        options,
      );
      if (response.ok) {
        const responseJson = await response.json();
    ...
    <Box>
      ...
      <Box display="flex" flexDirection={isMobile ? 'column' : 'row'} marginBottom="1rem">
        ...
        <div className="compare-button-container">
          {compare.length === 1 && (
          <p className="compare-button-helper-text">
